...
hide if Allt. status is Confirmed, Cancelled, Withdrawn  
dashboard should not change count
...
for(int i=0; i<list.size(); i++){
    ...
    if(
        ...
        !"Withdrawn".equalsIgnoreCase(rowStatus)
...
class="<%= 
    ...
    "Widthdrawn".equalsIgnoreCase(val(row.get("Status_Allot"))) ? "rej-row" : 
    	"Cancelled".equalsIgnoreCase(val(row.get("Status_Allot"))) ? "rej-row" :
