...
@WebServlet("/SeatAllotmentReport")
public class SeatAllotmentReport extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection con = DBUtil3.getConnection()) {
        	...
        	String sql = "SELECT * FROM admission_form " +
                    "WHERE TRIM(Status_Allot) = 'Confirmed' " +
                    "ORDER BY Seat_Allot ASC, CAST(Total AS DECIMAL(10,2)) DESC";
        ...
        request.getRequestDispatcher("SeatAllotmentReport.jsp").forward(request, response);
