<%@ page import="java.util.*" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Admission Records</title>

<%
HttpSession sess = request.getSession(false);
if (sess == null || sess.getAttribute("username") == null) {
    response.sendRedirect("login.jsp");
    return;
}
%>

<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/css/bootstrap.min.css">

<style>
body { font-family: Arial; background:#f4f6f9; margin:10px; }
table { width:100%; background:white; }
th,td { border:1px solid #ddd; padding:6px; font-size:12px; white-space:nowrap; }
th { background:#f3f3f3; }
.table-wrapper { overflow-x:auto; }
</style>
</head>

<body>

<%@ include file="header.jsp" %>

<h4>SANPOLY - Admission Records</h4>

<div class="table-wrapper">
<table class="table table-bordered table-sm">

<thead>
<tr>
<th>ID</th>
<th>APPNO</th>
<th>Categ. No</th>
<th>Name</th>
<th>DOB</th>
<th>Gender</th>
<th>Admission</th>

<th>Father Name</th>
<th>F Occ</th>
<th>F Org</th>

<th>Mother Name</th>
<th>M Occ</th>
<th>M Org</th>

<th>Income</th>
<th>Address<th>

<th>Phone</th>
<th>Whatsapp</th>
<th>Email</th>

<th>SSLC State</th>
<th>Aadhar</th>
<th>APAAR</th>


<th>Board</th>
<th>Medium</th>
<th>Year</th>

<th>Maths</th>
<th>Science</th>
<th>Total</th>
<th>Aggr</th>

<th>P1</th>
<th>P2</th>
<th>P3</th>
<th>P4</th>
<th>P5</th>

<th>Created</th>
<th>Action</th>
</tr>
</thead>

<tbody>

<%
List<Map<String, Object>> list = (List<Map<String, Object>>) request.getAttribute("data");

if (list != null) {
    for (Map<String, Object> row : list) {
%>

<tr>

<td><%= row.get("id") == null ? "" : row.get("id") %></td>
<td><%= row.get("APPNO") == null ? "" : row.get("APPNO") %></td>
<td><%= row.get("cast_no") == null ? "" : row.get("cast_no") %></td>
<td><%= row.get("applicant_name") == null ? "" : row.get("applicant_name") %></td>
<td><%= row.get("date_of_birth") == null ? "" : row.get("date_of_birth") %></td>
<td><%= row.get("gender") == null ? "" : row.get("gender") %></td>
<td><%= row.get("Admission_type") == null ? "" : row.get("Admission_type") %></td>

<td><%= row.get("father_guardian_name") == null ? "" : row.get("father_guardian_name") %></td>
<td><%= row.get("father_occupation") == null ? "" : row.get("father_occupation") %></td>
<td><%= row.get("Father_org") == null ? "" : row.get("Father_org") %></td>

<td><%= row.get("mother_name") == null ? "" : row.get("mother_name") %></td>
<td><%= row.get("mother_occupation") == null ? "" : row.get("mother_occupation") %></td>
<td><%= row.get("Mother_org") == null ? "" : row.get("Mother_org") %></td>

<td><%= row.get("income") == null ? "" : row.get("income") %></td>

<td><%= row.get("postal_address") == null ? "" : row.get("postal_address") %></td>


<td><%= row.get("phone_no") == null ? "" : row.get("phone_no") %></td>
<td><%= row.get("Whatsapp_no") == null ? "" : row.get("Whatsapp_no") %></td>
<td><%= row.get("email") == null ? "" : row.get("email") %></td>

<td><%= row.get("SSLC_State") == null ? "" : row.get("SSLC_State") %></td>
<td><%= row.get("aadhar_no") == null ? "" : row.get("aadhar_no") %></td>
<td><%= row.get("APAAR_ID") == null ? "" : row.get("APAAR_ID") %></td>

<td><%= row.get("SSLC_Board") == null ? "" : row.get("SSLC_Board") %></td>
<td><%= row.get("medium_of_instruction") == null ? "" : row.get("medium_of_instruction") %></td>
<td><%= row.get("sscl_passing_year") == null ? "" : row.get("sscl_passing_year") %></td>

<td><%= row.get("marks_maths") == null ? "" : row.get("marks_maths") %></td>
<td><%= row.get("marks_science") == null ? "" : row.get("marks_science") %></td>
<td><%= row.get("SSLC_TMarks") == null ? "" : row.get("SSLC_TMarks") %></td>
<td><%= row.get("SSLC_Aggr") == null ? "" : row.get("SSLC_Aggr") %></td>

<td><%= row.get("preference_1") == null ? "" : row.get("preference_1") %></td>
<td><%= row.get("preference_2") == null ? "" : row.get("preference_2") %></td>
<td><%= row.get("preference_3") == null ? "" : row.get("preference_3") %></td>
<td><%= row.get("preference_4") == null ? "" : row.get("preference_4") %></td>
<td><%= row.get("preference_5") == null ? "" : row.get("preference_5") %></td>

<td><%= row.get("created_at") == null ? "" : row.get("created_at") %></td>

<td>
<button class="btn btn-primary btn-sm" onclick="editRecord(this)"
data-id="<%= row.get("id") %>"
data-name="<%= row.get("applicant_name") %>"
>Edit</button>
</td>

</tr>

<%
    }
}
%>

</tbody>
</table>
</div>

</body>
</html>